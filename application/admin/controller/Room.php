...
class Room extends Controller
{
    ...
    public function index() {
        ...
        $hotelModel = new PlaceModel();
        $hotelList = $hotelModel->where("type = 1 and status = 0")->select();
    ...
    public function delOne() {
        ...
        $postData = $request->param();
        $id = $postData['id'];
        if(ItemModel::destroy(array("id" => $id))) {
            exit(json_encode(array('state' => 1, 'msg' => 'success')));
        } else {
    ...
    public function editStatus() {
        ...
        if (ItemModel::update(array('status' => $status), array('id' => $postData['id']))) {
            exit(json_encode(array('state' => 1, 'msg' => 'success')));
        } else {
